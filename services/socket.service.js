...
function setupSocketAPI(http) {
  ...
  gIo.on('connection', socket => {
    ...
    socket.on("new set", (savedSet) => {
      if (savedSet.user) {
        console.log('LINE 23 SAVED SET', savedSet)
        socket.to(savedSet.seller._id).emit("added set", savedSet);
      }
      // socket.to(savedSet).emit("set received");
    })
    socket.on("new status", ({ set, notification }) => {
      socket.to(set.user._id).emit("changed status", set);
      socket.to(set.user._id).emit("set status", notification);
    })
